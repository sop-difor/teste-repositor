...
| F7 | **concluída e publicada em produção (05/09/2026)** — 4/4 `APROVADO` ([`fase-7-revisao.md`](fase-7-revisao.md)). 👍/👎 nas respostas; painel de uso/falhas (`assistente-painel.html`); purga automática de 180 dias (LGPD, prometida na F1) via `pg_cron`. Migração aplicada (coluna `veredito`, índice, job `gecope-assistente-purga-log` ativo); Edge Function `gecope-assistant` v20→v22; `gecope-assistant-painel` publicada pela primeira vez (v1). Conferido byte a byte contra o repositório após o deploy, e confirmado ao vivo pelo usuário: botão 👍 gravou o voto de verdade ("Obrigado pelo retorno."), e o painel mostrou o mesmo voto refletido em "1 Feedback positivo" |
| F8 | **concluída — 4/4 `APROVADO` + sign-off do usuário (05/09/2026)** ([`fase-8-revisao.md`](fase-8-revisao.md)). Cartão "Assistente de Dados" no menu principal (`index.html`); tema claro/escuro unificado com o resto do GECOPE; saudação com o nome real do usuário logado; brasão do Ceará no cabeçalho e na tela de boas-vindas; barra de rolagem padronizada; respostas do caminho da IA em tabela de verdade (colunas alinhadas). **Roteiro F0-F8 concluído — piloto liberado** |
